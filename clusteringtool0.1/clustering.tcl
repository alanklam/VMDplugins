...
proc clustering::cluster {} {
  ...
  checkbutton $w.result.options.1.join -text "Join 1 member clusters" -variable clustering::join_1members -command [namespace code UpdateLevels]
  ...
  pack $w.result.options.2.rep_type -side top -anchor nw
  ...
  checkbutton $w.result.options.2.keep -text "Keep current representations" -variable clustering::keep_rep -command [namespace code UpdateLevels]
...
# v0.1: added option to update on top of current representations, option menu to choose representations
...
proc clustering::add_rep {num name} {
  ...
  # offset num by the current no. of repsentations
  set repnum [expr $num + [molinfo $clust_mol get numreps]]
  mol rep $rep_type
...
#############################################################################
# Calculate
# v0.1: changed representation to Licorice
